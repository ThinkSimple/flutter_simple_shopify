...
import '../../graphql_operations/mutations/checkout_shipping_address_update.dart';
...
class ShopifyCheckout with ShopifyError {
  ...
  Future<Checkout> getCheckoutInfoQuery(String checkoutId, {bool deleteThisPartOfCache = false}) async {
    ...
    final WatchQueryOptions _options = WatchQueryOptions(
        documentNode:
            gql(_requiresShipping(result) == true ? getCheckoutInfo : getCheckoutInfoWithoutShipping),
        variables: {
          'id': checkoutId,
        });
  ...
  Future<List<Order>> getAllOrders(String customerAccessToken,
    ...
    final QueryOptions _options = WatchQueryOptions(
        ...
        variables: {'accessToken': customerAccessToken, 'sortKey': sortKey.parseToString()});
    final QueryResult result = await ShopifyConfig.graphQLClient.query(_options);
    checkForError(result);
    Orders orders =
        Orders.fromJson(((((result?.data ?? const {}))['customer'] ?? const {})['orders'] ?? const {}));
    if (deleteThisPartOfCache) {
      _graphQLClient.cache.write(_options.toKey(), null);
    }
  ...
  Future<void> checkoutLineItemsReplace(String checkoutId, List<String> variantIdList,
  ...
  List<Map<String, dynamic>> transformVariantIdListIntoListOfMaps(List<String> variantIdList) {
    ...
    variantIdList.forEach((e) {
      if (lineItemList.indexWhere((test) => e == test['variantId']) == -1)
        lineItemList
            .add({"quantity": variantIdList.where((id) => e == id).toList().length, "variantId": e});
    });
    return lineItemList;
  }
  ...
  Future<void> checkoutCustomerDisassociate(String checkoutId,
      {bool deleteThisPartOfCache = false}) async {
    final MutationOptions _options = MutationOptions(
        documentNode: gql(checkoutCustomerDisassociateMutation), variables: {'id': checkoutId});
    final QueryResult result = await _graphQLClient.mutate(_options);
    checkForError(result);
    if (deleteThisPartOfCache) {
      _graphQLClient.cache.write(_options.toKey(), null);
  ...
  Future<void> checkoutDiscountCodeRemove(String checkoutId,
      {bool deleteThisPartOfCache = false}) async {
    final MutationOptions _options = MutationOptions(
        documentNode: gql(checkoutDiscountCodeRemoveMutation), variables: {'checkoutId': checkoutId});
  ...
  Future<String> createCheckout({bool deleteThisPartOfCache = false}) async {
    ...
    return ((result?.data['checkoutCreate'] ?? const {})['checkout'] ?? const {})['id'];
  }
  ...
  Future<void> checkoutGiftCardRemove(String appliedGiftCardId, String checkoutId,
    ...
    final MutationOptions _options = MutationOptions(
        ...
        variables: {'appliedGiftCards': appliedGiftCardId, 'checkoutId': checkoutId});
    final QueryResult result = await _graphQLClient.mutate(_options);
    checkForError(result);
    if (deleteThisPartOfCache) {
      _graphQLClient.cache.write(_options.toKey(), null);
  ...
  /// Updates [Checkout] shipping address of given [checkoutId]
  Future<void> checkoutShippingAddressUpdate(String checkoutId,
      {String address1,
      ...
      bool deleteThisPartOfCache = false}) async {
    final _options =
        MutationOptions(documentNode: gql(checkoutShippingAddressUpdateMutation), variables: {
      'checkoutId': checkoutId,
      'address1': address1,
      'address2': address2,
      'company': company,
